...
$connect_params = " -host http://xnat.cci.emory.edu:8080/xnat -u admin -p admin ";

$BASE_COMMAND = " /home/dgutman/xnat_tools/XNATRestClient $connect_params ";

$current_project="DG_XNAT_PROD";

 my $csv = Text::CSV->new();


##/data/archive/projects/PROJECT_ID/pipelines/triggerPipelines/experiments/EXPT_ID

$RERUN_PIPELINE = "/data/archive/projects/${current_project}/pipelines/triggerPipelines/experiments/CCI_XNAT_PROD_E00184";

#$UPDATE_SCAN_ID = "$EXPERIMENT_URI/scans/$SCAN_ID?quality=$QUALITY&type=$NEW_NAME";

$FULL_SYNTAX = $BASE_COMMAND . " -m PUT -remote \"" . $RERUN_PIPELINE ."\"";;
