...
POST_GD_MASK_PATH = '/IMAGING_SCRATCH/TCGA_DICOM_CACHE/TCGA_DATA/NIFTI_FILES/NIFTI_MASKS/POST_GD_MASKS/'
...
#Path to the AXIAL FLAIR IMAGES 
FLAIR_IMAGE_PATH = '/IMAGING_SCRATCH/TCGA_DICOM_CACHE/TCGA_DATA/NIFTI_FILES/RAW_NIFTI_FILES/AXIAL-T2-FLAIR/'
#Path to the AXIAL FLAIR MASKS
FLAIR_MASK_PATH = '/IMAGING_SCRATCH/TCGA_DICOM_CACHE/TCGA_DATA/NIFTI_FILES/NIFTI_MASKS/POST_GD_MASKS/'
...
#make a list of all nii.gz masks for the flair images
for file_path in glob.glob(FLAIR_MASK_PATH + '*'):
    path = file_path.split('/')
    if path[len(path)-1].endswith('.nii.gz'):
        flair_images.append(path[len(path)-1])
...
for file_path in glob.glob(POST_GD_MASK_PATH + '*'):
    path = file_path.split('/')
    if path[len(path)-1].endswith('.nii.gz'):
        post_gd_masks.append(path[len(path)-1])





for mask in post_gd_masks:
    ...
    if m.group(1):
        ...
        author = m.group(4)
    print 'post gd mask id is ', patient_id, 'author is ', author  
    continue
