#!/usr/bin/python
import glob, os, tempfile, re, sys
...
BACKGROUNDS_DIR =       "/IMAGING_SCRATCH/TCGA_DICOM_CACHE/TCGA_DATA/PLASTIMATCH_MASK_EXPORT/BACKGROUNDS/"
...
def process_dicom_rt_header_file(input_dicom_rt_file, converted_file_name):
    ...
    if found_structure_set:
        ...
        if os.path.exists(series_uid_dir):
            print BACKGROUNDS_DIR+converted_file_name+'_BACKGROUND.nii.gz'
            command = '/IMAGING_SCRATCH/TCGA_DICOM_CACHE/SCRIPTS/plastimatch/build/plm/plastimatch convert --input '+series_uid_dir+' --output-img '+BACKGROUNDS_DIR+converted_file_name+'_BACKGROUND.nii.gz'
            os.system(command)
            
            tempDir = tempfile.mkdtemp()
            command = '/IMAGING_SCRATCH/TCGA_DICOM_CACHE/SCRIPTS/plastimatch/build/plm/plastimatch convert --input '+input_dicom_rt_file+' --output-prefix '+tempDir+' --fixed '+ series_uid_dir
            os.system(command)
            #convert each structure in the DIOCM RT object
            for struct in glob.glob(tempDir+'/*'):
                struct_name = os.path.split(struct)[1]
                #struct will be a .mha file

                file_name = CONVERSION_OUTPUT_DIR+converted_file_name+'_'+struct_name[:-4].replace('_','-')
                command = 'plastimatch convert --input '+ struct +' --output-img '+file_name+'.nii.gz'
                print command
                os.system(command)
...
for each in dcm_rt_file_list:
    ...
    converted_file_name = subject_id.replace('_','-')+'_'+file_type.replace('_','-')+'_'+file_name[:-4].replace('_','-')
